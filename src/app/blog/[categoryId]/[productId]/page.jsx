...
const AnimatedClientComponentBlog = dynamic(() => import("./AnimatedClientComponentBlog"), {
  ssr: false,
});
import { getPosts } from "@/app/lib/firebase/postBlog/read_server";
...
export default async function Page({ params }){
  ...
  const postDate = post.timestamp.toDate().toLocaleDateString('en-US', {day:'2-digit', month:'long', year:'numeric'});
  //  {Array.isArray(post) && post?.map(user => {
  //   return(
  //     <span>{user?.timestamp?.toDate()?.toLcaleDateString()}</span> // this returns the date string
  //   )
  // })}
  return <>
  <AnimatedClientComponentBlog post={post} matchCateName={matchCateName} postDate={postDate}/>
    {/* <Script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.9.1/gsap.min.js" />
    <Script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.9.1/ScrollTrigger.min.js" />
    <Script src="/js/panels.js" strategy="afterBlogactive" /> */}
