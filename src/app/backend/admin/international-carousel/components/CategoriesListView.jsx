...
import { useCategories } from "@/app/lib/firebase/international-carousel/read";
            ...
            <tbody>
        {
            data?.map((item, key) => {
                return <tr key={item.name}>
                    ...
                    <td>{item?.name}</td>
                    <td>{item?.slug}</td>
                    <td>{item?.content && <div dangerouslySetInnerHTML={{__html: item?.content}} className="font10"></div>}</td>
                    <td>{item?.link}</td>
                    <td>
                        {item?.timestamp.toDate().toLocaleTimeString('en-US', {day:'2-digit', month:'2-digit', year:'numeric'})}                    
                    </td>
                    <td><Link href={`/backend/admin/international-carousel/form?id=${item?.id}`} className="btn">Edit</Link></td>
                </tr>
            })
        }
        </tbody>
